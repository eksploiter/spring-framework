...
import com.ssafy.live.controller.BootAuthController;
...
@Slf4j
@WebMvcTest(value = BootAuthController.class)
...
public class AuthControllerTest {
    ...
    private Member createTestMember(String email) {
        return Member.builder().email(email).password("password").name("테스터").role("MANAGER").build();
    }

    private MockHttpServletRequestBuilder sessionRequest(String path, String method, String email) {
        CustomUserDetails details = new CustomUserDetails(createTestMember(email));
        MockHttpServletRequestBuilder builder = method.equals("GET") ? MockMvcRequestBuilders.get(path)
                : MockMvcRequestBuilders.post(path);
        //builder.sessionAttr("loginUser", loginUser);
        // CustomUserDetails 사용 후 설정
        builder.with(user(details));
        return builder;
    }
    ...
    void memberDetail_get요청_성공_model에member와api키존재_viewName_member_detail() throws Exception {
        ...
        var request = sessionRequest("/auth/member-detail", "GET", "admin@ssafy.com").param("email", email);
    ...
    void memberList_get요청_key존재_성공_model에page존재_viewName_member_list() throws Exception {
        ...
        var request = sessionRequest("/auth/member-list", "GET",
                "admin@ssafy.com").param("key", "1").param("word", "테스터")
                        .param("currentPage", "1");
    ...
    void memberModifyForm_get요청_성공_model에member존재_viewName_member_modify_form() throws Exception {
        ...
        String email = "test@test.com";
        Member member = createTestMember(email);
        var request = sessionRequest("/auth/member-modify-form", "GET", "admin@ssafy.com").param("email", email);
        // When & Then
        when(mService.selectDetail(email)).thenReturn(member);
        mock.perform(request).andExpect(status().isOk()).andExpect(model().attribute("member", member))
    ...
    void memberModify_post요청_성공_redirect_memberDetail_session업데이트() throws Exception {
        // Given
        String email = "test@test.com";
        // 로그인 사용자를 이용해야 하므로 customuserdetails를 생성
        CustomUserDetails userDetails = new CustomUserDetails(createTestMember("test@ssafy.com"));
        var request = sessionRequest("/auth/member-modify", "POST",
                "admin@ssafy.com").param("email", email).param("name", "hong")
                        .param("password", "1234").with(user(userDetails));
        // When & Then
    ...
    void memberModify_post요청_성공_다른회원수정_redirect_memberDetail_session유지() throws Exception {
        ...
        var request = sessionRequest("/auth/member-modify", "POST",
                "admin@ssafy.com").param("email", modifyEmail).param("name", "테스터")
                        .param("password", "password"); // custom user 등록
        // When & Then
        doNothing().when(mService).update(any(Member.class)); // void 메서드는 doNothing() 사용
        mock.perform(request).andExpect(status().is3xxRedirection())
    ...
    void memberModify_post요청_실패_SQLException_redirect_memberDetail_alertMsg존재() throws Exception {
        ...
        var request = sessionRequest("/auth/member-modify", "POST",
                "admin@ssafy.com").param("email", email).param("name", "테스터")
    ...
    void memberDelete_post요청_로그인회원삭제성공_redirect_root_session무효화() throws Exception {
        ...
        var request = sessionRequest("/auth/member-delete", "POST",
                "admin@ssafy.com").param("mno", String.valueOf(member.getMno()))
                        .param("email", email).with(user(details));
        // When & Then
        mock.perform(request).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/member/logout"))
    ...
    void memberDelete_post요청_다른회원삭제성공_redirect_memberList_alertMsg존재() throws Exception {
        ...
        var request = sessionRequest("/auth/member-delete", "POST",
                "admin@ssafy.com").param("mno", String.valueOf(mno))
    ...
    void addressInsert_post요청_성공_ResponseBody_memberAddresses반환() throws Exception {
        ...
        var request = sessionRequest("/auth/address-insert", "POST",
                "admin@ssafy.com").param("email", email).param("address", "서울")
    ...
    void addressDelete_post요청_성공_ResponseBody_memberAddresses반환() throws Exception {
        ...
        when(mService.selectDetail(email)).thenReturn(member);
        var request = sessionRequest("/auth/address-delete", "POST",
                "admin@ssafy.com").param("email", email).param("ano",
                        String.valueOf(ano));
