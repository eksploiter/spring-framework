...
public class BasicMemberDao implements MemberDao {
    ...
    @Override
    // TODO: 15-2. BLob 형태로 데이터를 조회하는 부분을 살펴보자.
    public Member selectDetail(Connection con, String email) throws SQLException {
        ...
        try (PreparedStatement pstmt = con.prepareStatement(sql)) {
            ...
            while (rset.next()) {
                ...
                byte[] profile = rset.getBytes("profile");
                if (member == null) {
                    member = Member.builder().mno(mno).name(name).email(email).password(pass).role(role)
    ...
    // TODO: 15-3. BLob 형태로 데이터를 등록하는 부분을 살펴보자.
    @Override
    public int updateProfile(Connection con, String email, byte[] profile) throws SQLException {
        int result = 0;
        String sql = "update member set profile=? where email=?";
        try (PreparedStatement pstmt = con.prepareStatement(sql)) {
            pstmt.setBytes(1, profile);
